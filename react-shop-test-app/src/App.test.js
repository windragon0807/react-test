...
test("From order to order completion", async () => {
    ...
    expect(screen.getByText("America x 2")).toBeInTheDocument();
    expect(screen.getByText("England x 3")).toBeInTheDocument();
    ...
    const productsTotal = screen.getByText("여행 상품 가격: 0");
    expect(productsTotal).toBeInTheDocument();

    const optionsTotal = screen.getByText("여행 옵션 가격: 0");
